...
def send_login_email(user_email, user_name):
    """Send actual login notification email"""
    try:
        # Create message
        msg = MIMEMultipart('alternative')
        msg['Subject'] = "🔐 Login Alert - Flood Prediction System"
        msg['From'] = app.config['MAIL_USERNAME']
        msg['To'] = user_email
        ...
        <body>
            <h2>Hello {user_name},</h2>
            <p>You have successfully logged into your Flood Prediction System account.</p>
            
            <div class="alert">
                <h3>📱 Login Details:</h3>
                <p><strong>Time:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
                <p><strong>IP Address:</strong> {request.remote_addr if request else 'N/A'}</p>
            </div>
            
            <p>If this was not you, please contact support immediately.</p>
            
            <p>Stay safe,<br>
            <strong>Flood Prediction Security Team</strong></p>
        </body>
        </html>
        """
        ...
        #print(f"❌ Login email sending failed: {e}")
        return False
    
def send_alert_email(user_email, user_name, risk_category):
    """Send flood risk alert email"""
    try:
        if risk_category == "High":
            title = "🚨 High Flood Risk"
            intro = (
                "Water levels are close to the danger mark in your region. "
                "Please prepare and stay alert for any official instructions."
            )
            advice = (
                "Review evacuation routes, keep emergency kits ready, and "
                "avoid activities near the river or reservoir."
            )
        else:  # Critical
            title = "🛑 Critical Flood Risk"
            intro = (
                "Water levels are extremely close to or above danger levels. "
                "There is a serious risk of flooding in your region."
            )
            advice = (
                "Move to higher ground if advised, keep important documents with you, "
                "and follow all instructions from local authorities immediately."
            )

        msg = MIMEMultipart('alternative')
        msg['Subject'] = f"Flood Alert ({risk_category}) - Flood Prediction System"
        msg['From'] = app.config['MAIL_USERNAME']
        msg['To'] = user_email
        ...
        <body>
            ...
            <div class="alert">
                ...
                <p><strong>Time:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
                <p>{advice}</p>
                <p>Please open your dashboard for detailed dam and weather information.</p>
            </div>
...
def user_dashboard():
    ...
    try:
        if (risk_info
            ...
            # optional: avoid spamming by remembering last level in session
            last_level = session.get('last_risk_level')
            if last_level != risk_info['level']:
                send_alert_email(user.email, user.name,risk_info['level'])  # or a separate alert_email()
                session['last_risk_level'] = risk_info['level']
    except Exception:
        # swallow email errors so dashboard still loads
        pass
